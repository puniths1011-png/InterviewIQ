...
const parseGroqResponse = (text) => {
  try {
    ...
    console.error('Error parsing Groq JSON:', error);
...
const callChatCompletion = async (prompt, isJson = true) => {
  const completion = await groq.chat.completions.create({
    messages: [
      { role: 'system', content: isJson ? 'You are a helpful assistant that responds only in JSON.' : 'You are a helpful assistant.' },
...
Return ONLY a valid JSON array with this exact structure:
{
  ...
  const responseText = await callChatCompletion(prompt, true);
...
- Be professional and objective.`;
  ...
  return await callChatCompletion(prompt, false);
};
